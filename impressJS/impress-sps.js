 ...
 *  downloadtime: 2016-5-24
 *  comment by Pushu Shi
 */
...
( function( document, window ) {
    ...
    //
    // pfx 是一个根据所运行的浏览器给予标准的css属性（即加上前缀）
    var pfx = ( function() {
    ...
    // arraify把类数组对象转化成正真的数组。使得数组原型上方法可用
    // Array.prototype.slice.call(arguments)能将具有length属性的对象转成数组
    var arrayify = function( a ) {
        return [].slice.call( a ); //call指向传进来的对象
    };
    ...
    // given as `fallback`).
    // toNumber将给的参数‘numbric’转化成数字，如果无法转化就返回0，或者回调函数
    var toNumber = function( numeric, fallback ) {
        return isNaN( numeric ) ? ( fallback || 0 ) : Number( numeric );//三元运算，test ? expression1 : expression2，test为true返回ex1，false则返回ex2
    };

    // `byId` returns element with given `id` - you probably have guessed that ;)
    // 简化document.getElementById(),等同jquery里$('#id');
    ...
    // $(selector,context)返回第一个含有‘selector’的元素
    ...
    // the given element or whole document.
    // $(selector,context)返回一个所有含有‘selector’的元素的数组
    var $$ = function( selector, context ) {
        context = context || document;
    ...
    var triggerEvent = function( el, eventName, detail ) {
        var event = document.createEvent( "CustomEvent" ); //创建自定义事件
        event.initCustomEvent( eventName, true, true, detail );
        el.dispatchEvent( event ); //向一个指定的 EventTarget 派发一个事件,  以合适的顺序触发受影响的 EventListeners
    };
    ...
    var ua = navigator.userAgent.toLowerCase();//获取浏览器类型
    var impressSupported =
                          ...
                          // dataset 属性允许我们访问所有在元素上自定义的data属性 (这种属性一般以data-开头），它的结构是一个DOMString映射表
                          ...
                          // 因为一些移动端队css的3d属性支持不够好而且硬件也不够运行impress的渲染，所以加入下面一行代码来屏蔽移动端
                           ( ua.search( /(iphone)|(ipod)|(android)/ ) === -1 );//删除此行可以使impress在mobile端使用

    if ( !impressSupported ) {

        // We can't be sure that `classList` is supported
        body.className += " impress-not-supported ";//浏览器不支持以上条件的话，给body加上‘impress-not-supported’这个class
    } else {
    ...
    var impress = window.impress = function( rootId ) {
        ...
        var initStep = function( el, idx ) {
            var data = el.dataset,
                step = {
                    translate: {
                        x: toNumber( data.x ),//把data-x的值转化为数字
                        y: toNumber( data.y ),
                        z: toNumber( data.z )
                    },
                    rotate: {
                        x: toNumber( data.rotateX ),//把data-rotate-x的值转化为数字
        ...
        var init = function() {
            ...
            // For some reason iPad goes nuts when it is not done properly.什么原因？
            ...
            var rootData = root.dataset; // 298 line: root = byId(rootid)
            config = {
            ...
            // 299line: cavas = document.createElement('div')
